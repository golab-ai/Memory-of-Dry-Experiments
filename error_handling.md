...
yield_comparison返回0/0成功：可能因下载数据不完整、LCMS方法未正确识别目标产物或数据文件命名不符合规范。建议：验证实验目录下原始文件完整性，确认分析方法与产物分子量匹配，检查文件命名规则。
...
结果0/0成功表示未生成任何有效比较项。常见原因：实验轮次名称与实际目录不匹配、LCMS数据缺失或峰积分失败。解决：先用file_match检查各轮实验文件列表，确保两轮共有的样品名存在，再重新触发分析。
...
- 问题：产率分析返回 0/0 成功，表明未找到可对比的产率数据
- 可能原因：实验记录文件中缺失产率字段、文件名不匹配、目录结构与预期不符
- 解决方案：检查分析目录下文件命名规范，确认每轮实验记录中包含 'yield' 或类似字段；若数据缺失，需补全后再触发分析
...
尽管任务状态成功，但产率对比返回0条成功记录。可能原因：数据目录下文件缺失、命名不规范或元数据不完整。改进方向：在pipeline前期增加目录结构与数据完整性校验，若匹配文件为空则明确提示用户检查数据准备情况。
...
分析结果为0/0成功，表示未找到可对比的产率数据。常见原因：产物命名不一致导致无匹配项；实验数据中缺少产率信息（未计算或未导出）。应提示用户确认产物列表是否包含产率字段。
...
当产率比较工具返回 0/0 成功时，表示无有效产率数据可用于对比，可能原因：实验数据缺失、批次未正确匹配或产率计算失败。需检查分析目录下实验文件完整性及命名规范。
...
- 分析结果为0/0成功，可能因文件匹配阶段未找到两轮实验的LCMS数据文件
- 建议：在用户prompt中明确指定实验文件夹名称或完整路径，避免模糊描述
- 未来可增加预检查步骤，若文件缺失则提示用户补充数据
...
yield_comparison阶段返回0/0成功，表明无有效产率数据可比较。常见原因是实验数据中缺少产率字段或file_match未正确关联两轮实验文件。需提示用户确认实验记录是否完整，并检查数据文件格式是否符合预期。
...
yield_comparison 返回 0/0 成功时，通常表示两轮实验中没有可比较的产率数据对，可能原因：1) 实验数据未正确下载或格式不符；2) 文件匹配失败，找不到对应的产物；3) LCMS 执行未产生定量结果。建议先单独检查每轮实验的产率报告是否完整。
...
结果返回 0/0 成功表明产率比较步骤未获得有效数据对。常见原因：① 实验批次中缺失产物质量或理论产量字段；② 文件匹配阶段未能识别 LCMS 定量文件（如 .txt 或 .csv 格式不匹配）；③ 两轮实验的产物名称或批号不一致导致无法对应。解决方案：检查原始实验记录是否录入产率相关数值，确认 LCMS 解析器能提取定量信息，并确保 file_match 阶段的正则规则覆盖数据文件格式。
...
- **问题**: yield_comparison返回0/0成功，无有效比较结果。
- **排查**: 检查数据文件格式、列名是否与工具预期一致；确认两轮实验ID映射正确；验证文件内容非空且包含产率数值。
- **解决**: 标准化列名，统一实验标识，重新运行或人工校验数据文件。
...
分析返回0/0成功时，可能原因是：1）两轮实验中未找到共同产物；2）LCMS数据缺失产率字段；3）文件匹配失败。建议后续检查实验数据完整性和产物命名一致性。
...
问题：分析结果 0/0 成功，表现为无有效比较产生。原因：可能实验目录中缺少部分轮次的产物信息或 LCMS 原始文件，导致 yield_comparison 找不到成对数据。解决方案：提示用户检查‘分析目录’下是否每轮都包含完整的产物重量记录或对应的 LCMS 文件，并确认文件命名符合匹配规则；执行前可先预览 file_match 结果以确保所有轮次数据就绪。
...
若结果返回0/0成功，通常表示没有找到可比较的产率对。常见原因：分析目录中缺失必需的文件（如原始LCMS数据、积分文件）、两轮实验的产物命名不一致导致文件匹配失败、或数据格式不符合工具预期。解决方案：检查目录结构，确认两轮实验数据均完整且使用统一的命名规则；必要时手动指定匹配对。
...
0/0成功：常见于无可匹配文件或数据缺失产率列。排查：1) file_match规则是否遗漏文件；2) LC-MS原始数据是否包含产率信息；3) 实验ID或路径是否正确。优先建议用户确认数据完整性。
...
产率对比返回 0/0 成功，通常因分析目录中缺少匹配的 LCMS 原始数据或对应轮次的实验记录。需检查：1) 分析目录是否已正确挂载并包含数据文件；2) 文件命名是否符合实验标签；3) lcms_execute 是否成功运行并输出结果。建议在流程前增加目录内容校验与友好提示。
...
当 yield_comparison 返回 0/0 成功时，通常因 file_match 未找到任何匹配的文件对。检查分析目录路径是否正确、文件命名是否包含可识别的轮次前缀（如 Round1/Round2）。若文件存在仍失败，可尝试单独运行 lcms_execute 验证单轮数据完整性。
...
• 必需列缺失：若xlsx中无'Yield'列，抛出明确错误并提示检查列名。
• 数据泄露：验证unseen配体标识符(如Canonical SMILES或ID)确保不在训练集中出现，防止评估虚高。
• 分子图生成异常：若传入的SMILES无效，捕获RDKit SanitizationError，记录警告并跳过该配体，不中断流程。
• 实验室提交失败：采用重试机制（最多3次），失败后记录详细日志并通知人工介入。
...
- **文件路径错误**: 确认 xlsx 文件与脚本同目录，使用绝对路径时注意转义
- **SMILES无效**: 用 RDKit 的 MolFromSmiles 验证，剔除无法解析的配体并记录日志
- **重复配体**: 在训练前按规范形式 SMILES 去重，避免数据泄漏
- **预测失败**: 若 unseen set 为空，提示用户无可用新配体，终止筛选
