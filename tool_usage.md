...
- 模型训练: 使用自定义多性质预测训练框架，输入分子描述符，输出Tg/Tm/Td三个回归目标，训练配置epochs=50，批次大小32，优化器Adam，损失MSE。
- 虚拟筛选: 调用批量预测模块，对/mnt/nas/.../p11下的筛选分子库生成预测值，然后按Tg≥250, Tm≥300, Td≥450并行过滤，输出符合条件分子列表。
...
- 模型训练: 使用 model_trainer 工具，设置 epochs=50，输入训练数据路径 /mnt/nas/.../p11。
- 虚拟筛选: 使用 virtual_screener 工具，加载训练好的模型，设定筛选条件为 Tg>=250, Tm>=300, Td>=450。
...
使用AI训练管道：加载/mnt/nas/.../p12数据，训练回归模型（epoch=50），预测分子库渗透性能。筛选工具接收多条件字典：{O2: ≥5 Barrer, N2: bottom 50, CO2: ≥50 Barrer, CH4: bottom 50, H2: ≥100 Barrer}，自动排序并返回符合条件的分子。
...
ts_generation阶段使用 xtb 的 GFN2-xTB 方法结合反应物与产物结构生成初始过渡态猜测（如通过扫描键长或调用 autoTS 模块）。dft_optimization阶段使用 ORCA 在 B3LYP/def2-SVP 级别进行过渡态优化（关键词 OptTS, Freq），并运行频率计算验证唯一虚频。
...
- experiment_download: 从/mnt/nas/opencode_data_2/runjob/目录下载实验结果
- file_match: 匹配目录中的配置文件与数据文件
- lcms_execute: 以batch模式对1个样本执行LC-MS分析
- yield_comparison: 对比产率数据，输出1/1成功
...
lcms_execute: 核心分析工具，需传入数据目录和样本ID，自动检测同目录下的配置文件。file_match: 前置校验，确保所需原始数据文件与配置文件存在且匹配。experiment_download: 从远程拉取实验包。yield_comparison: 后处理，生成样本产率对比。
...
- experiment_download: skipped (0 experiments)
- file_match: matched sample ID to data files using directory scan and config
- lcms_execute: processed LC-MS raw data with default integration parameters
- yield_comparison: compared yield metrics against reference
  Key params: --data_dir, --config_dir, --sample_id; successful when directory naming and config format follow conventions.
...
使用了yield_comparison工具进行产率对比。关键参数：需指定实验目录及分析模式为'yield_comparison'。前置工具experiment_download用于获取实验数据，file_match用于定位目标文件，lcms_execute预处理LCMS数据。注意确保file_match正确识别待比较的文件。
...
file_match: 从指定配置文件自动匹配样本二的数据文件，目录与配置同路径。
lcms_execute: 执行LC-MS分析，使用匹配到的数据文件。
yield_comparison: 对比产率结果，生成分析报告。
关键参数：结果目录、配置文件路径、样本标识。
...
experiment_download: 从指定路径加载第二轮实验数据。file_match: 根据配置文件自动匹配样本二在新实验D下的数据文件。lcms_execute: 执行液相色谱-质谱联用数据分析，提取峰面积等参数。yield_comparison: 对比产量结果，生成样本二在不同实验间的优化报告。
...
- experiment_download: 从user提供的目录加载原始数据
- file_match: 依据样本ID（样本二）和实验名（新实验D）关联配置文件与数据
- lcms_execute: 针对匹配到的数据运行LC-MS分析
- yield_comparison: 对比产率，生成最终分析结论
...
- experiment_download: 从提供的目录下载实验配置文件
- file_match: 将配置中的文件与实际数据文件匹配
- lcms_execute: 执行LCMS数据分析，提取峰值和产率
- yield_comparison: 对比新旧配体在不同实验中的产率，生成比较结果
上述工具串联使用，确保从配置下载到产率对比的端到端分析，适用于单样本批处理模式。
...
- experiment_download: 从远程存储拉取指定轮次实验数据目录
- file_match: 基于实验ID匹配对应的配置文件（实验D）
- lcms_execute: 调用LC-MS分析模块，处理新配体色谱峰与质谱数据
- yield_comparison: 计算并对比目标产物的产率或转化率
...
使用yield_comparison分析模式：自动从分析目录读取第一、二轮实验数据；依赖experiment_download确保数据就绪，file_match进行轮次关联，lcms_execute处理色谱数据，最终生成产率对比。
...
- experiment_download: 用于从指定目录获取实验结果
- file_match: 自动匹配目录中的配置文件（如实验参数、化合物信息）
- lcms_execute: 执行 LCMS 数据处理，提取目标配体信息
- yield_comparison: 对比新配体在实验D中的产率变化
...
- experiment_download: 自动下载实验记录；参数根据实验ID或目录。
- file_match: 匹配第一轮与第二轮实验文件，关键参数：轮次标签，匹配字段（产品名/结构）。
- lcms_execute: 执行LCMS数据处理，获取面积归一化产率。
- yield_comparison: 对比产率，参数：至少两轮结果路径，输出差异报告。
...
experiment_download: 用于获取实验原始数据。
lcms_execute: 执行LCMS分析计算产率。
yield_comparison: 多轮产率对比。注意：需确保数据路径和文件名匹配。
...
experiment_download: 按轮次名称下载实验数据。file_match: 关联原始文件与样品元数据，通过实验标签过滤。lcms_execute: 使用yield_comparison分析模式，输入需指定两轮实验标识符。yield_comparison: 直接对比两轮产率结果。
...
- 主要工具：yield_comparison
- 参数：实验记录所在目录路径（如分析目录），通常自动从文件匹配阶段获取
- 成功用法：确保目录中存在标准格式的实验记录文件（如 CSV/JSON 包含产量字段）
...
experiment_download: 根据指定分析目录自动下载第一、二轮实验数据；file_match: 匹配两轮实验中的目标化合物数据文件；lcms_execute: 对匹配文件执行LCMS分析；yield_comparison: 汇总并对比两轮的产率结果，支持批量计算。关键参数：分析目录路径、轮次标识（如round1/round2）。
...
- experiment_download: 根据实验ID获取原始数据包
- file_match: 匹配LCMS数据文件与产物名称
- lcms_execute: 提取LCMS数据中目标产物的峰面积或纯度
- yield_comparison: 对比两轮实验中同一产物的产率，计算变化值
...
- lcms_execute: 用于执行LCMS数据提取，为产率计算准备原料与产物峰面积。
- yield_comparison: 对多个实验的产率进行横向比较，自动匹配相同反应条件的实验批次。
...
- experiment_download：用于获取实验原始数据，需确保实验ID或目录路径正确
- lcms_execute：执行LCMS数据分析，但本次调用可能因文件匹配失败未产生有效结果
- 产率比较依赖文件匹配步骤，需提前检查数据文件完整性
...
涉及工具：lab_plan（规划实验步骤）、experiment_download（下载实验数据）、file_match（匹配第一、二轮实验文件）、lcms_execute（处理LCMS数据）和yield_comparison（执行产率对比）。yield_comparison依赖前序工具产出的数据文件，若上游数据缺失则返回空结果。
...
- experiment_download: 下载指定轮次的实验原始数据
- file_match: 匹配与分析目录下的数据文件，确保文件对应正确
- lcms_execute: 执行 LCMS 数据处理或峰积分，生成定量结果
- yield_comparison: 输入两轮实验 ID，进行产率比较，返回成功/失败计数
...
lab_plan: 用于生成分析计划，需指定 analysis_mode='yield_comparison' 及实验批次ID；
experiment_download: 拉取指定实验的原始数据；
file_match: 将下载文件与已知数据格式对齐；
lcms_execute: 执行 LCMS 数据解析，可能需设置产物保留时间等参数；
yield_comparison: 对比多组实验的产率，期望输入为解析后的定量结果。
...
- **yield_comparison**: 用于对比多轮实验产率。需提供两轮实验的数据文件路径或ID。
- **file_match**: 匹配分析目录中的实验记录文件，确保文件存在且格式正确。
- **success**: 成功执行但返回0/0成功结果，表明工具运行正常但无有效数据被处理。
...
● experiment_download：指定轮次参数（rounds: [1,2]）拉取实验数据。
● file_match：按产物名或ID匹配两轮实验的对应文件。
● lcms_execute：对匹配到的文件进行LCMS计算，得到产率。
● yield_comparison：汇总并对比产出率，关键参数：analysis_type='yield'。
...
lcms_execute：用于执行 LCMS 分析，需提供正确的数据文件路径和产物 m/z 参数。yield_comparison：依赖前一步的输出，要求产物信息完整。file_match：确保能匹配到所有轮次的目标文件，否则下游无法产生有效比较。experiment_download：从指定目录获取实验数据，需要准确的目录路径。
...
experiment_download: 下载指定分析目录下的实验数据；file_match: 匹配两轮实验的对应产物或条件文件；lcms_execute: 执行液相-质谱数据处理；yield_comparison: 计算并比较产率，需提供两轮匹配文件路径。典型参数：round1_id, round2_id, analysis_dir。
...
使用experiment_download获取两轮实验原始数据 → file_match筛选产率相关文件（如含Yield列的数据表） → lcms_execute执行LC-MS分析 → yield_comparison计算产率变化。需确保文件命名包含轮次信息。
...
- yield_comparison: 在 lab_analysis 流程末端调用，对比两轮实验的产率，但需上游 file_match 成功匹配所需的 LCMS 数据文件。
- file_match: 依赖实验元数据匹配分析目录下的数据文件，若目录为空或文件命名不规范会导致 0/0 成功。
...
- file_match: 用于自动匹配第一轮与第二轮实验的峰面积文件，需保证文件命名包含轮次标识且位于指定的分析目录下。
- lcms_execute: 在匹配成功后对每轮数据分别执行 LCMS 分析，生成产率中间结果。
- yield_comparison: 最终汇聚两轮结果进行产率对比，依赖前两步的输出。
...
- 产率模型训练：使用ligand_optimizer，输入文件ligand-match-fix_replaced.xlsx，自动划分训练/测试集，支持回归或分类目标。
- 虚拟筛选：用模型对unseen配体库打分，返回top-k推荐。
- 分子图生成：调用mol_drawer，批量绘制推荐配体的2D结构，保存为图像。
- 实验提交：通过lab_submit将排名第一的配体编码和元数据发送至湿实验系统。
...
• 数据加载：pandas 读取 ligand-match-fix_replaced.xlsx
• 分子编码：RDKit Morgan 指纹（半径=2, 2048位）
• 产率模型：RandomForestRegressor（n_estimators=200, random_state=42）
• 虚拟筛选：对未见过配体编码后使用模型预测产率，按降序取前5
• 分子图生成：rdkit.Chem.Draw.MolsToGridImage 绘制推荐配体结构
• 实验室提交：调用 lab_submit(reagent_id='ligand_1', amount='5mg') 发送最高分配体
...
• load_data: 加载ligand-match-fix_replaced.xlsx，自动识别配体特征列与产率列(Yield)。
• train_yield_model: 使用随机森林回归器训练产率预测模型，目标列='Yield'。
• predict_unseen: 对配体库中未见过的(unsenn)分子进行批量预测，按预测产率降序排序，取top-k。
• draw_molecule: 为每个推荐配体生成分子结构图，方便化学家评估。
• submit_to_lab: 将排名第一的配体信息（ID、预测产率、分子图）发送至实验室消息队列或邮件，触发湿实验。
...
• load_data: pandas.read_excel 读取 ligand-match-fix_replaced.xlsx
• featurize: RDKit 生成 Morgan 指纹（radius=2, nBits=2048）及物化描述符
• train_model: sklearn.ensemble.RandomForestRegressor (n_estimators=100, random_state=42)
• predict: 模型预测未见配体产率，按产率降序取 top-k
• visualize: RDKit MolToImage 生成推荐配体分子结构图
• submit: 实验室信息管理系统 API 提交实验请求
...
• pandas: 读取 ligand-match-fix_replaced.xlsx 数据，快速构建特征表。
• RDKit: 从SMILES生成分子对象，计算描述符（如MolLogP、TPSA等），并使用 Draw.MolsToGridImage 批量生成推荐配体的结构图。
• scikit-learn: 使用 RandomForestRegressor 训练产率预测模型，对未见配体产率进行排序筛选。
• matplotlib: 辅助数据可视化与结果展示。
• 成功用法：将配体结构描述符与反应条件合并为输入矩阵，模型训练后预测未见配体产率，取Top-K推荐。
...
- **data_loader**: 加载 `ligand-match-fix_replaced.xlsx`，提取 SMILES 与产率标签
- **yield_predictor**: 训练随机森林模型，输入分子指纹（ECFP4, 2048 bits），输出产率预测
