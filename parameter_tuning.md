...
• 随机森林：n_estimators=100，max_depth=None，min_samples_split=2，random_state=42，使用默认参数即可获得稳定性能。
• 数据预处理：数值特征用StandardScaler标准化，缺失值用中位数填充，分类变量进行one-hot编码。
• 筛选策略：只保留预测产率高于训练集产率中位数的候选配体，避免低潜候选进入实验。
• 交叉验证：采用5折CV评估模型R²，确保无严重过拟合。
...
使用随机森林默认 n_estimators=100 可获得稳定基线；结合 5-fold 交叉验证对 n_estimators（100~300）和 max_depth（5~20）调优，通常 n_estimators=200、max_depth=None 能较好平衡偏差与方差，同时避免过拟合。
...
• 特征工程：使用RDKit计算配体的2D描述符（分子量、LogP、氢键供体/受体等），并结合已有的反应条件（温度、时间等）作为模型输入。
• 模型配置：采用随机森林回归，n_estimators=100，max_depth=10，random_state=42，以兼顾性能与过拟合风险（可通过GridSearchCV进一步寻优）。
• 数据拆分：训练/测试集按80/20划分，确保产率分布近似。
• 推荐数量：筛选预测产率Top-5未见配体，生成结构图并发送实验室验证。
...
- **模型参数**: RandomForestRegressor(n_estimators=200, max_depth=8, random_state=42)
- **分子指纹**: Morgan 指纹 radius=2, nBits=2048
- **筛选阈值**: 预测产率 > 60% 或 top 20 unseen 配体
- **可视化**: 子图大小 (300,300), kekulize=True, 标注原子索引
...
• 分子指纹：半径2，2048位Morgan指纹，能较好表征膦配体结构
• 模型：Random Forest，n_estimators=200，max_depth=15，min_samples_leaf=3，防止过拟合
• 筛选Top k：推荐3-5个未见配体，兼顾多样性和高预测产率
• 交叉验证：5-fold CV评估模型R²>0.7后再用于虚拟筛选
...
模型超参数采用贝叶斯优化：n_estimators 100~500, max_depth 5~15, learning_rate 0.01~0.2。数据划分使用 StratifiedGroupKFold 保持产率分组。特征选择通过 SHAP importance 筛选 top 15 重要描述符，减少过拟合并提升泛化能力。未见配体筛选仅保留预测产率前 10 且具有合成可行性的分子。
...
模型训练：采用RandomForestRegressor，n_estimators=100，max_depth=10，random_state=42，经5折交叉验证确认无过拟合。筛选：选取预测产率最高的5个unseen配体。结构图生成：设置图像尺寸(300,300)，原子标注，清除氢原子以便清晰展示。
